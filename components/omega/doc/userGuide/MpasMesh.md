...
on the sphere. An attribute `sphere_radius` is required that defines the
radius of the sphere. The MPAS mesh specification places no requirement on
that radius, but Omega does: a spherical mesh must lie on a sphere with the
Earth radius from the Physical Constants Dictionary (`mean_radius`,
6371008.7714 m), and Omega aborts if `sphere_radius` differs from it by more
than a relative tolerance of $10^{-8}$. Meshes generated by Polaris satisfy
this; older MPAS-Ocean meshes (typically 6371229 m) must be rescaled, which
Polaris' `utils/omega/convert_mpaso_ic_to_omega.py` does along with the
variable renaming.
...
| `sphere_radius` | double    | any non-negative real value                   | Radius of the sphere the points are defined on. Set to 0 for a planar mesh. Omega requires the PCD Earth radius (6371008.7714 m) for spherical meshes. |
