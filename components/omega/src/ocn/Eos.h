...
/// Small forward-mode dual number carrying the first derivatives of a TEOS-10
/// quantity with respect to the normalized temperature (DT, = d/dTt) and
/// normalized salinity (DS, = d/dSs) variables used by the 75-term polynomial.
/// Using this type in place of Real in the coefficient assembly yields the
/// analytic first derivatives at no extra TEOS-10 evaluation cost.
struct EosDeriv {
   Real V;  ///< value
   Real DT; ///< derivative with respect to Tt = Ct / CtNorm
   Real DS; ///< derivative with respect to Ss = sqrt((Sa + DeltaS) / SaNorm)

   KOKKOS_INLINE_FUNCTION EosDeriv() : V(0), DT(0), DS(0) {}
   KOKKOS_INLINE_FUNCTION EosDeriv(Real Val) : V(Val), DT(0), DS(0) {}
   KOKKOS_INLINE_FUNCTION EosDeriv(Real Val, Real Dt, Real Ds)
       : V(Val), DT(Dt), DS(Ds) {}
};

KOKKOS_INLINE_FUNCTION EosDeriv operator+(const EosDeriv &A,
                                          const EosDeriv &B) {
   return EosDeriv(A.V + B.V, A.DT + B.DT, A.DS + B.DS);
}
KOKKOS_INLINE_FUNCTION EosDeriv operator*(const EosDeriv &A,
                                          const EosDeriv &B) {
   return EosDeriv(A.V * B.V, A.DT * B.V + A.V * B.DT, A.DS * B.V + A.V * B.DS);
}

/// Assemble the six TEOS-10 pressure-polynomial coefficients (Roquet et al.
/// 2015, 75-term expansion) from the normalized temperature Tt and salinity Ss.
/// Templated on the scalar type so it can be evaluated either with Real (to get
/// the coefficients themselves, used by computeSpecVol) or with EosDeriv (to
/// obtain their first derivatives by forward-mode differentiation, used by
/// computeSpecVolAndDerivs). The Real instantiation is bit-for-bit identical to
/// the original inline assembly in Teos10Eos::calcPCoeffs.
template <typename T>
KOKKOS_FUNCTION void eosSpecVolPolyCoeffs(T (&C)[6], const T Tt, const T Ss) {
   ...
   C[5] = V005;
   C[4] = V014 * Tt + V104 * Ss + V004;
   C[3] = (V023 * Tt + V113 * Ss + V013) * Tt + (V203 * Ss + V103) * Ss + V003;
   C[2] =
       (((V042 * Tt + V132 * Ss + V032) * Tt + (V222 * Ss + V122) * Ss + V022) *
            Tt +
        ((V312 * Ss + V212) * Ss + V112) * Ss + V012) *
           Tt +
       (((V402 * Ss + V302) * Ss + V202) * Ss + V102) * Ss + V002;
   C[1] = ((((V051 * Tt + V141 * Ss + V041) * Tt + (V231 * Ss + V131) * Ss +
             V031) *
                Tt +
          ...
          V001;
   C[0] = (((((V060 * Tt + V150 * Ss + V050) * Tt + (V240 * Ss + V140) * Ss +
              V040) *
                 Tt +
...
class Teos10Eos {
 public:
   ...
   /// TEOS-10 helpers
   /// Calculate pressure polynomial coefficients for TEOS-10. Thin wrapper that
   /// normalizes (Ct, Sa) and delegates the 75-term assembly to the templated
   /// eosSpecVolPolyCoeffs (single source of truth, shared with the derivative
   /// path). The result is bit-for-bit identical to the previous inline form.
   KOKKOS_FUNCTION void calcPCoeffs(Real (&SpecVolPCoeffs)[6 * VecLength],
      ...
      Real C[6];
      eosSpecVolPolyCoeffs<Real>(C, Tt, Ss);
      for (int I = 0; I < 6; ++I)
         SpecVolPCoeffs[I + 6 * KVec] = C[I];
   }

   /// Compute specific volume together with its first derivatives with respect
   /// to conservative temperature, absolute salinity, and pressure, all
   /// evaluated at the reference state (Ct, Sa, p) of each cell/layer. The
   /// specific volume itself is computed by the exact same code path as
   /// operator() so it is bit-for-bit identical to computeSpecVol; the three
   /// derivatives are obtained analytically from a single coefficient assembly
   /// via forward-mode differentiation (no extra TEOS-10 polynomial evaluation,
   /// satisfying the bounded-EOS-cost requirement). Derivatives are returned
   /// per physical unit: d(SpecVol)/d(ConservTemp) [m^3 kg^-1 degC^-1],
   /// d(SpecVol)/d(AbsSalinity) [m^3 kg^-1 (g/kg)^-1], and
   /// d(SpecVol)/d(Pressure) [m^3 kg^-1 Pa^-1].
   KOKKOS_FUNCTION void
   specVolAndDerivs(const Array2DReal &SpecVol, const Array2DReal &DThetaCons,
                    const Array2DReal &DSalt, const Array2DReal &DPressure,
                    I4 ICell, I4 KChunk, const Array2DReal &ConservTemp,
                    const Array2DReal &AbsSalinity,
      ...
      // Reference-profile pressure-polynomial coefficients (mirror
      // calcRefProfile); used only for the pressure derivative.
      constexpr Real R00 = -4.4015007269e-05;
      constexpr Real R01 = 6.9232335784e-06;
      constexpr Real R02 = -7.5004675975e-07;
      constexpr Real R03 = 1.7009109288e-08;
      constexpr Real R04 = -1.6884162004e-08;
      constexpr Real R05 = 1.9613503930e-09;
      ...
      for (int KVec = 0; KVec < KLen; ++KVec) {
         ...
         // Specific volume: identical code path to operator() (bit-for-bit)
         calcPCoeffs(SpecVolPCoeffs, KVec, Ct, Sa);
         SpecVol(ICell, K) =
             calcRefProfile(Pdb) + calcDelta(SpecVolPCoeffs, KVec, Pdb);

         // First derivatives by forward-mode differentiation of the same
         // 75-term polynomial. Seed Tt with d/dTt=1 and Ss with d/dSs=1.
         const Real Ss = Kokkos::sqrt((Sa + DeltaS) / SaNorm);
         const Real Tt = Ct / CtNorm;
         EosDeriv C[6];
         eosSpecVolPolyCoeffs<EosDeriv>(C, EosDeriv(Tt, 1.0_Real, 0.0_Real),
                                        EosDeriv(Ss, 0.0_Real, 1.0_Real));
         const Real Pp = Pdb * PNorm;
         const EosDeriv Delta =
             ((((C[5] * Pp + C[4]) * Pp + C[3]) * Pp + C[2]) * Pp + C[1]) * Pp +
             C[0];

         // Chain rule from (Tt, Ss) back to (Ct, Sa); the reference profile has
         // no Ct/Sa dependence so it does not contribute to these derivatives.
         DThetaCons(ICell, K) = Delta.DT / CtNorm;
         DSalt(ICell, K)      = Delta.DS / (2.0_Real * SaNorm * Ss);

         // Pressure derivative (per Pa) of refProfile + delta. Pp = p * Pa2Db *
         // PNorm, so d/dp = d/dPp * PNorm * Pa2Db.
         const Real DDeltaDPp =
             C[1].V +
             Pp * (2.0_Real * C[2].V +
                   Pp * (3.0_Real * C[3].V +
                         Pp * (4.0_Real * C[4].V + Pp * 5.0_Real * C[5].V)));
         const Real DRefDPp =
             R00 +
             Pp * (2.0_Real * R01 +
                   Pp * (3.0_Real * R02 +
                         Pp * (4.0_Real * R03 +
                               Pp * (5.0_Real * R04 + Pp * 6.0_Real * R05))));
         DPressure(ICell, K) = (DDeltaDPp + DRefDPp) * PNorm * Pa2Db;
      }
   }
...
class LinearEos {
 public:
   ...
   /// Compute linear specific volume and its first derivatives. SpecVol matches
   /// operator() bit-for-bit. The derivatives are analytic:
   /// d(1/rho)/dx = -(drho/dx)/rho^2, with drho/dCt = DRhodT, drho/dSa =
   /// DRhodS, and no pressure dependence.
   KOKKOS_FUNCTION void
   specVolAndDerivs(const Array2DReal &SpecVol, const Array2DReal &DThetaCons,
                    const Array2DReal &DSalt, const Array2DReal &DPressure,
                    I4 ICell, I4 KChunk, const Array2DReal &ConservTemp,
      ...
      for (int KVec = 0; KVec < KLen; ++KVec) {
         ...
         const Real Sv =
             1.0_Real / (RhoT0S0 + (DRhodT * ConservTemp(ICell, K) +
                                    DRhodS * AbsSalinity(ICell, K)));
         SpecVol(ICell, K)    = Sv;
         DThetaCons(ICell, K) = -DRhodT * Sv * Sv;
         DSalt(ICell, K)      = -DRhodS * Sv * Sv;
         DPressure(ICell, K)  = 0.0_Real;
      }
   }
...
class ConstantEos {
 public:
   ...
   /// Compute constant specific volume and its (identically zero) first
   /// derivatives. SpecVol matches operator() bit-for-bit.
   KOKKOS_FUNCTION void
   specVolAndDerivs(const Array2DReal &SpecVol, const Array2DReal &DThetaCons,
                    const Array2DReal &DSalt, const Array2DReal &DPressure,
                    I4 ICell, I4 KChunk, const Array2DReal &ConservTemp,
      ...
      for (int KVec = 0; KVec < KLen; ++KVec) {
         const I4 K           = KStart + KVec;
         SpecVol(ICell, K)    = 1.0_Real / RhoSw;
         DThetaCons(ICell, K) = 0.0_Real;
         DSalt(ICell, K)      = 0.0_Real;
         DPressure(ICell, K)  = 0.0_Real;
      }
   }
...
class Eos {
 public:
   ...
   Array2DReal
       SpecVolDThetaCons;    ///< d(SpecVol)/d(ConservTemp) at level centers
   Array2DReal SpecVolDSalt; ///< d(SpecVol)/d(AbsSalinity)  at level centers
   Array2DReal
       SpecVolDPressure; ///< d(SpecVol)/d(Pressure)     at level centers
   ...
   /// Compute specific volume AND its first derivatives with respect to
   /// conservative temperature, absolute salinity, and pressure, in a single
   /// pass. SpecVol is bit-for-bit identical to computeSpecVol; the derivatives
   /// are written into SpecVolDThetaCons, SpecVolDSalt, and SpecVolDPressure.
